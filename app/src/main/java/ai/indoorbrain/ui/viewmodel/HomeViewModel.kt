...
class HomeViewModel(
    ...
    private var buttonPressFlowJob: Job? = null
    private var autoOfflineJob: Job? = null
    private var pendingSyncMonitorJob: Job? = null
    ...
    private fun updateAvailableActions(presses: List<ButtonPress>) {
        ...
        val lastAction = lastPress?.let { 
            try {
                ...
                Log.e("HomeViewModel", "❌ Invalid action: ${it.action}")
                null
            }
        }
        ...
        pendingSyncMonitorJob = viewModelScope.launch {
            while (true) {
                try {
                    ...
                    Log.e("HomeViewModel", "Error checking pending sync count", e)
                }
        ...
        autoOfflineJob = viewModelScope.launch {
            while (true) {
                ...
                if (_isOnline.value) {
                    ...
                    if (timedOut) {
                        Log.w("HomeViewModel", "User auto-offlined due to inactivity")
    ...
    fun toggleOnlineStatus() {
        viewModelScope.launch {
            ...
            if (newState) {
                // Going online
                Log.d("HomeViewModel", "User went ONLINE")
                preferencesManager.updateLastActivity()
            } else {
                // Going offline
                Log.d("HomeViewModel", "User went OFFLINE")
            }
        }
    }
    ...
    fun onButtonPress(action: ButtonAction, floorIndex: Int? = null) {
        viewModelScope.launch {
            Log.d("HomeViewModel", "🔘 ${action.name}${if (floorIndex != null) " (Floor $floorIndex)" else ""}")
            ...
            try {
                ...
                if (result.isFailure) {
                    Log.e("HomeViewModel", "❌ Button save failed: ${result.exceptionOrNull()?.message}")
                ...
                Log.e("HomeViewModel", "❌ Button error: ${e.message}")
                _errorMessage.value = e.message
            } finally {
    ...
    private fun controlIMUDataCapture(action: ButtonAction, sessionId: String) {
        val intent = Intent(context, IMUDataService::class.java)
        
        when (action) {
            ButtonAction.LEFT_RESTAURANT_BUILDING -> {
                Log.d("HomeViewModel", "📊 START 2-min capture")
                intent.action = IMUDataService.ACTION_START_CAPTURE_TIMED
                intent.putExtra(IMUDataService.EXTRA_DURATION_MS, 2 * 60 * 1000L)
                intent.putExtra("session_id", sessionId)
                context.startService(intent)
            }
            
            ButtonAction.REACHED_SOCIETY_GATE -> {
                Log.d("HomeViewModel", "📊 START continuous capture")
                intent.action = IMUDataService.ACTION_START_CAPTURE_CONTINUOUS
                intent.putExtra("session_id", sessionId)
                context.startService(intent)
            }
            
            ButtonAction.LEAVING_SOCIETY -> {
                Log.d("HomeViewModel", "📊 START 3-min capture (keeping sessionId for final data)")
                intent.action = IMUDataService.ACTION_START_CAPTURE_TIMED
                intent.putExtra(IMUDataService.EXTRA_DURATION_MS, 3 * 60 * 1000L)
    ...
    fun cancelSession() {
        viewModelScope.launch {
            try {
                ...
                Log.e("HomeViewModel", "❌ Cancel error: ${e.message}")
