...
class AuthViewModel(
    ...
    fun login(phone: String, password: String) {
        viewModelScope.launch {
            ...
            if (result.isSuccess) {
                ...
                Log.d("AuthViewModel", "Login successful: ${result.getOrNull()?.fullName}")
            } else {
                _errorMessage.value = result.exceptionOrNull()?.message ?: "Login failed"
                Log.e("AuthViewModel", "Login failed", result.exceptionOrNull())
            }
            ...
            while (true) {
                ...
                if (user != null) {
                    ...
                    if (timeUntilExpiry < 10 * 60 * 1000L) {
                        Log.d("AuthViewModel", "Token expiring soon, refreshing...")
    ...
    private suspend fun refreshTokenIfNeeded() {
        ...
        if (isExpired) {
            Log.d("AuthViewModel", "Token expired, attempting refresh...")
            ...
            if (result.isSuccess) {
                ...
                Log.e("AuthViewModel", "Token refresh failed, logging out user")
                logout()
            }
        }
    }
